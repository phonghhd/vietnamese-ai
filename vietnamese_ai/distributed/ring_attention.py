...
class RingAttentionNode:
    ...
    def forward_ring(
        self, Q_local: np.ndarray, K_local: np.ndarray, V_local: np.ndarray
    ) -> np.ndarray:
        """
        ...
        O = np.zeros_like(Q_local) # noqa: E741
        m = np.full((batch_size, local_seq_len, 1), -np.inf)
        l = np.zeros((batch_size, local_seq_len, 1)) # noqa: E741
        ...
        for step in range(self.total_nodes):
            ...
            m, l, O = self._local_attention_step(Q_local, current_K, current_V, m, l, O) # noqa: E741
