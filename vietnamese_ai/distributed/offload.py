...
class OffloadEngine:
    ...
    def tinh_toan_device_map(
        ...
        vram_cho_phep = max_gpu_vram_gb if max_gpu_vram_gb else (gpu_vram_thuc * 0.9) # Dùng tối đa 90%
        
        if model_size_gb <= vram_cho_phep:
            logger.info(f"Mô hình ({model_size_gb:.1f}GB) vừa với GPU VRAM ({vram_cho_phep:.1f}GB). Không cần Offload.")
            return {"device_map": {"": 0}} # Chạy toàn bộ trên GPU 0
            
        logger.warning(f"Mô hình ({model_size_gb:.1f}GB) vượt quá GPU VRAM ({vram_cho_phep:.1f}GB). Kích hoạt CPU Offloading!")
