...
class TheoDoiGPU:
    ...
    def _tai_thu_vien_nvml(self):
        ...
        try:
            if sys.platform.startswith("win"):
                nvml_path = os.path.join(os.environ.get("ProgramW6432", "C:\\Program Files"), "NVIDIA Corporation", "NVSMI", "nvml.dll")
            ...
            logger.warning(f"[TheoDoiGPU] Không tìm thấy thư viện NVML hoặc máy không có GPU NVIDIA. Lỗi: {e}")
    ...
    def _lay_thong_tin_mot_gpu(self, index: int) -> Dict[str, Any]:
        ...
        ret = self._nvml_lib.nvmlDeviceGetHandleByIndex_v2(ctypes.c_uint(index), ctypes.byref(handle))
        if ret != NVML_SUCCESS:
            return {}
        ...
        self._nvml_lib.nvmlDeviceGetTemperature(handle, ctypes.c_uint(NVML_TEMPERATURE_GPU), ctypes.byref(nhiet_do))
