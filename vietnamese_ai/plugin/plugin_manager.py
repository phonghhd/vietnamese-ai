...
from vietnamese_ai.sandbox import PhanTichAST, LoiAnNinh
...
class QuanLyPlugin:
    ...
    def nap_plugin(self, duong_dan_file: str, bo_qua_kiem_duyet: bool = False) -> Optional[PluginCoSo]:
        """
            ...
            for ten_thanh_phan, kieu_thanh_phan in inspect.getmembers(module_nap, inspect.isclass):
                ...
                if issubclass(kieu_thanh_phan, PluginCoSo) and kieu_thanh_phan is not PluginCoSo:
                    ...
                    if thuc_the_plugin.khoi_dong():
                        ...
                        logger.info(f"[Plugin] Đã nạp thành công: {thuc_the_plugin.ten} v{thuc_the_plugin.phien_ban}")
                        return thuc_the_plugin
                    else:
                        logger.error(f"[Plugin] Hàm khoi_dong() của '{thuc_the_plugin.ten}' trả về False.")
                        self.go_plugin(ten_module) # Dọn dẹp
                        return None
                        
            logger.error(f"[Plugin] Không tìm thấy Class nào kế thừa PluginCoSo trong file '{duong_dan_file}'")
