...
class DeepSeekMoE:
    ...
    """
    def __init__(self, d_model: int, num_shared_experts: int = 1, num_routed_experts: int = 16, top_k: int = 4):
        ...
        self.shared_proj = [
            LopDense(d_model * 2, d_model) for _ in range(num_shared_experts)
        ]
        ...
        self.routed_proj = [
            LopDense(expert_dim, d_model) for _ in range(num_routed_experts)
        ]
