...
class PowerManager:
    ...
    def get_battery_status() -> Tuple[float, bool]:
        ...
        """
        if not _CO_PSUTIL:
            logger.warning("Cần cài đặt psutil (pip install psutil) để đọc pin. Mặc định trả về 100% sạc.")
            return 100.0, True
    ...
    def dieu_tiet_do_chinh_xac(cls) -> Dict[str, Any]:
        ...
        logger.warning(f"Pin yếu (Pin {percent}%). KÍCH HOẠT CHẾ ĐỘ SINH TỒN: 1.58-bit (Tiết kiệm tối đa).")
        return {
            "precision": "1.58-bit",
            "load_in_4bit": False,
            "load_in_8bit": False,
