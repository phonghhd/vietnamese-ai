...
from vietnamese_ai.agents.swarm import HeThongSwarm, KetQuaSwarm, TacTuSwarm
...
class P2PSwarmOrchestrator(HeThongSwarm):
    ...
    def __init__(
        self, agent_khoi_tao: TacTuSwarm, tracker: P2PTracker, ledger: Optional[TokenLedger] = None
    ):
    ...
    def _chay_dong_thuan_bft(self, danh_sach_ket_qua: List[str]) -> str:
        ...
        prompt_trong_tai = (
            "Dưới đây là các câu trả lời từ nhiều Agent con khác nhau cho cùng một bài toán:\n\n"
        )
            ...
            while self.active_tasks:
                ...
                for future in done:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Tác tử '{meta['ten_sub_agent']}' bị lỗi (Node Crash): {str(e)}"
                        )
                        if meta["retries"] > 0:
                            logger.info(
                                f"Đang HỒI SINH (Auto-Respawn) tác tử '{meta['ten_sub_agent']}' sang Node khác... (Còn {meta['retries']} lần thử)"
                            )

                            new_node = self.tracker.tim_node_tot_nhat()
                            if new_node:
                                new_future = self.executor.submit(
                                    self._chay_node_tu_xa,
                                    new_node,
                                    meta["sub_agent"],
                                    meta["nhiem_vu"],
                                )
                                ...
                                results.append(
                                    f"Lỗi: Không còn Node rảnh để hồi sinh tác tử '{meta['ten_sub_agent']}'."
                                )
                        else:
                            results.append(
                                f"Tác tử '{meta['ten_sub_agent']}' đã CHẾT hẳn sau 3 lần thử."
                            )
