...
class MultiHeadFlashAttention:
    ...
    def tien(self, Q: np.ndarray, K: np.ndarray, V: np.ndarray) -> np.ndarray:
        ...
        # Biến O (Output), m (Max), l (Sum) sẽ được lưu trong SRAM (giả lập)
        O = np.zeros_like(Q_h)
        m = np.full((batch, so_dau, seq_len, 1), -np.inf)
        l = np.zeros((batch, so_dau, seq_len, 1))
        ...
        for j in range(0, seq_len, B_c):
            ...
            for i in range(0, seq_len, B_r):
                ...
                # 5. Cập nhật Output O_i
                O_i_prev = O[:, :, i:i+B_r, :]
                O_i_new = (factor * O_i_prev * l_i_prev + P_ij @ V_j) / (l_i_new + 1e-10)
        ...
        # Nối output lại
        concat = O.transpose(0, 2, 1, 3).reshape(batch, seq_len, self.d_model)
        return concat @ self.W_o
