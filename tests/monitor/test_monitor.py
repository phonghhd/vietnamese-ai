...
from vietnamese_ai.monitor import MayTinhTien, TheoDoiGPU, TheoDoiHeThong
...
def test_theo_doi_gpu_hoat_dong(mock_cdll):
    ...
    # Tên GPU
    mock_lib.nvmlDeviceGetName.side_effect = lambda h, name, length: (
        setattr(name, "value", b"RTX 3090 Ti") or 0
    )
    # Nhiệt độ 75C
    mock_lib.nvmlDeviceGetTemperature.side_effect = lambda h, st, t: (
        setattr(t._obj, "value", 75) or 0
    )
    # Công suất 250000 mW (250W)
    mock_lib.nvmlDeviceGetPowerUsage.side_effect = lambda h, p: (
        setattr(p._obj, "value", 250000) or 0
    )
